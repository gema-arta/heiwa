...
```bash
# Install.
time { cmake -B build -DCMAKE_INSTALL_PREFIX="/clang1-tools" -P build/cmake_install.cmake; }
```
...
> **Required!** For the most programs that depends on `-ltinfo` or `-lterminfo` linker's flags, including Stage-1 Clang/LLVM.
```bash
# Build.
