...
```bash
# The term variable is set to `xterm` rather than "$TERM" that broken when using `rxvt-unicode`. It's up to you.
...
time {
    ${CC} -flto=thin ${CFLAGS} ${CPPFLAGS} ${LDFLAGS} ../../extra/musl/files/musl-utils/getconf.c -o getconf && \
    ${CC} -flto=thin ${CFLAGS} ${CPPFLAGS} ${LDFLAGS} ../../extra/musl/files/musl-utils/getent.c -o getent   && \
    ${CC} -flto=thin ${CFLAGS} ${CPPFLAGS} ${LDFLAGS} ../../extra/musl/files/musl-utils/iconv.c -o iconv
}
...
grep --color=auto -E "CONFIG_TOYBOX_LIB(CRYPTO|Z)=y" .config
...
# Install and create symlink as `awk` that not linked since version links is disabled.
time {
    make install
...
> **Required!** Before `Linux`.
```bash
# Apply patch (from Alpine Linux) to allow search manually musl system paths (fallback).
