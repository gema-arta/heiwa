...
```
```bash
# Configure source. Ensure to link binaries to previously built musl libc.
cmake -B build \
    ...
    -DCMAKE_EXE_LINKER_FLAGS="-Wl,-dynamic-linker /cgnutools/lib/ld-musl-x86_64.so.1 $LDFLAGS"    \
    -DCMAKE_SHARED_LINKER_FLAGS="-Wl,-dynamic-linker /cgnutools/lib/ld-musl-x86_64.so.1 $LDFLAGS" \
