...
CFLAGS="-g0 $CFLAGS" CXXFLAGS="-g0 $CXXFLAGS" ../configure \
    ...
    --disable-{gdb,libdecnumber,multilib,nls,readline,sim,static,werror}
...
```
```bash
# GCC will looking for system headers in "/clang0-tools/usr/include/*", so create the directory, then symlink it.
...
# Remove those files.
find /clang0-tools/{lib64,{,${H_TRIPLET}/}lib,libexec}/ -name '*.la' -exec rm -rfv {} \;
